<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\FamousRoot $famousRoot
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Famous Root'), ['action' => 'edit', $famousRoot->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Famous Root'), ['action' => 'delete', $famousRoot->id], ['confirm' => __('Are you sure you want to delete # {0}?', $famousRoot->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Famous Roots'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Famous Root'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Users'), ['controller' => 'Users', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New User'), ['controller' => 'Users', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="famousRoots view large-9 medium-8 columns content">
    <h3><?= h($famousRoot->title) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('User') ?></th>
            <td><?= $famousRoot->has('user') ? $this->Html->link($famousRoot->user->username, ['controller' => 'Users', 'action' => 'view', $famousRoot->user->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Title') ?></th>
            <td><?= h($famousRoot->title) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Youtube') ?></th>
            <td><?= h($famousRoot->youtube) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($famousRoot->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Year') ?></th>
            <td><?= $this->Number->format($famousRoot->year) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($famousRoot->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($famousRoot->modified) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Body') ?></h4>
        <?= $this->Text->autoParagraph(h($famousRoot->body)); ?>
    </div>
</div>
